...
func TestSENDIQ_ValidateSampleRate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			sampleRate:  intPtr(250000),
			expectError: false,
		},
		{
			name:        "maximum valid sample rate",
			sampleRate:  intPtr(2000000),
			expectError: false,
		},
		{
			name:        "sample rate too low",
			sampleRate:  intPtr(5000),
			expectError: true,
			errorType:   commonerrors.ErrInvalidValue,
		},
		{
			name:        "sample rate too high",
			sampleRate:  intPtr(3000000),
			expectError: true,
			errorType:   commonerrors.ErrInvalidValue,
		},
	}
...
func TestSENDIQ_ValidateIQType(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "invalid type",
			iqType:      stringPtr("invalid"),
			expectError: true,
			errorType:   commonerrors.ErrInvalidValue,
		},
		{
			name:        "invalid type i32",
			iqType:      stringPtr("i32"),
			expectError: true,
			errorType:   commonerrors.ErrInvalidValue,
		},
		{
...
func TestSENDIQ_ValidatePower(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			power:         floatPtr(3.5),
			expectError:   false,
			expectedValue: floatPtr(3.5),
		},
		{
			name:          "minimum power",
			power:         floatPtr(0.0),
			expectError:   false,
			expectedValue: floatPtr(0.0),
		},
		{
			name:          "maximum power",
			power:         floatPtr(7.0),
			expectError:   false,
			expectedValue: floatPtr(7.0),
		},
		{
